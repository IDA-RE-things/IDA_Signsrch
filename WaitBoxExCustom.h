...
public:
	...
	void Begin(const char *format, ...)
	{
		...
		// Get wait box handle
		{
			...
			{
				FindWaitBox();
				if(!s_hWndWaitBox) Sleep(4);
			};
		}

		// Add progress bar to it
		if(s_hWndWaitBox)
		{			
			...
			if(!s_bIsQtVer)
			{
				...
				if(m_hWndProgress = CreateWindowEx(0, PROGRESS_CLASS, "", (WS_VISIBLE | WS_BORDER | WS_CHILD), ((283-BARWIDTH)/2)-2,20, BARWIDTH,17, s_hWndWaitBox, NULL, NULL, NULL))
				{
					...
					SendMessage(m_hWndProgress, PBM_SETBARCOLOR, 0, (LPARAM) RGB(100,210,0)); // Green RGB(0,210,0)
	...
	BOOL IsBreakProgress(int iPercent, LPSTR pszSegnemnt)
	{
		if(s_bStarted)
		{
			if(!s_bIsQtVer)
...
private:
	...
	{
		s_hWndWaitBox = NULL;
		EnumWindows(FindWaitBoxProc, (LPARAM) &s_hWndWaitBox);
	}
	//
	static BOOL CALLBACK FindWaitBoxProc(HWND hWnd, LPARAM lParam)
	{
		char szClass[max((sizeof("TWaitForm") + 1), (sizeof("QWidget") + 1))];
		if(GetClassName(hWnd, szClass, sizeof(szClass)))
		{
			if(s_bIsQtVer)
			{
				if(qstrcmp(szClass, "QWidget") == 0)
				{
					...
					{
						if(GetWindowLong(hWnd, GWL_STYLE) == 0x6CC0000)
						{
